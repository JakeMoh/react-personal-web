...
function Comment() {
  ...
  const [nameComment, setNameComment] = useState<CommentInfo[]>([]);
  ...
  async function postComment() {
    let jsonBody: CommentInfo = {name: userName, comment: userComment}
  ...
  function deleteComment(user_id: number) {
      ...
      arr.push(
        <tr key={row.user_id}>
          ...
          <td className="trash-button">
            <Button variant="danger" onClick={() => deleteComment(Number(row.user_id))}>
        ...
        <Form>
          <Form.Row>
            <Form.Group as={Col} md="4" controlId="formGroupEmail">
              ...
              <Form.Control placeholder="Enter Name" onChange={(event: any) => setUserName(String(event.target.value))}/>
            </Form.Group>
            <Form.Group as={Col} md="4" controlId="formGroupPassword">
              <Form.Label>Comment</Form.Label>
              <Form.Control placeholder="Enter Comment"  onChange={(event: any) => setUserComment(String(event.target.value))}/>
            </Form.Group>
          </Form.Row>
